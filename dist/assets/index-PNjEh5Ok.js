(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))o(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const s of a.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function r(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(i){if(i.ep)return;i.ep=!0;const a=r(i);fetch(i.href,a)}})();const P="tasktrack.tasks",w="tasktrack.boards",N="tasktrack.activeBoardId",Z="tasktrack.zoom",H=.2,R=2,D=.05,F=.0022,J=/(https?:\/\/[^\s<>"]+)/gi,K="Write the task here...",u=[],c=[];let m=null,g=1,O,h=null,A=null,x=null;const V=document.querySelector("#app"),U=t=>({id:t,title:`#${t}`,details:"",completed:!1,notes:[]}),L=(t,e,r=[],o=[],i=1)=>({id:t,name:e,tasks:r,trashedTasks:o,nextTaskNumber:i}),b=t=>t.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;"),C=t=>{const e=t??"";let r="",o=0;for(const i of e.matchAll(J)){const a=i.index??0,s=i[0];r+=b(e.slice(o,a)),r+=`<a href="${b(s)}" target="_blank" rel="noopener noreferrer">${b(s)}</a>`,o=a+s.length}return r+=b(e.slice(o)),r.replaceAll(`
`,"<br>")},Y=t=>t?.trim()?C(t):`<span class="task-links-placeholder">${b(K)}</span>`,X=t=>{const e=new Date(t);return Number.isNaN(e.getTime())?"":e.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})},Q=t=>t.length?t.toSorted((e,r)=>(e.createdAt??0)-(r.createdAt??0)).map(e=>`
      ...
      `).join(""):"",S=t=>Array.isArray(t)?t.map((e,r)=>({id:Number(e.id)||r+1,title:typeof e.title=="string"&&e.title.trim()?e.title:`#${r+1}`,details:typeof e.details=="string"?e.details:"",completed:!!e.completed,notes:Array.isArray(e.notes)?e.notes.map(o=>({text:typeof o?.text=="string"?o.text:"",createdAt:Number(o?.createdAt)||Date.now()})).filter(o=>o.text.trim()):[]})):[],B=(t,e=[])=>{const r=[...t,...e].reduce((o,i)=>Math.max(o,Number(i?.id)||0),0);return Math.max(1,r+1)},T=()=>c.find(t=>t.id===m)??c[0]??null,y=()=>{const t=T();if(!t){u.splice(0,u.length);return}u.splice(0,u.length,...S(t.tasks))},k=()=>{localStorage.setItem(w,JSON.stringify(c)),localStorage.setItem(N,String(m??""))},v=()=>{const t=T();t&&(t.tasks=S(u),localStorage.setItem(P,JSON.stringify(t.tasks)),k())},tt=()=>{try{const t=localStorage.getItem(w),e=Number(localStorage.getItem(N));if(t){const a=JSON.parse(t);if(Array.isArray(a)&&a.length){c.splice(0,c.length,...a.map((s,n)=>{const d=S(s?.tasks),l=S(s?.trashedTasks);return L(Number(s?.id)||n+1,typeof s?.name=="string"&&s.name.trim()?s.name:`Board ${n+1}`,d,l,Number(s?.nextTaskNumber)||B(d,l))})),m=Number.isFinite(e)?e:c[0].id,c.some(s=>s.id===m)||(m=c[0].id),y();return}}const r=JSON.parse(localStorage.getItem(P)??"[]"),o=S(r),i=L(1,"Board 1",o,[],B(o));c.splice(0,c.length,i),m=i.id,y(),k()}catch{localStorage.removeItem(w),localStorage.removeItem(N);const t=L(1,"Board 1",[],[],1);c.splice(0,c.length,t),m=t.id,y()}},et=()=>{const t=(c.at(-1)?.id??0)+1;c.push(L(t,`Board ${c.length+1}`,[],[],1)),m=t,y(),k(),f()},at=t=>{t===m||!c.some(e=>e.id===t)||(v(),m=t,y(),k(),f())},st=t=>{const e=c.find(o=>o.id===t);if(!e)return;const r=window.prompt("Gi board et navn",e.name)?.trim();r&&(e.name=r,k(),f())},rt=t=>{if(c.length<=1)return;const e=c.find(o=>o.id===t);if(!e||!window.confirm(`Slette board "${e.name}"?`))return;const r=c.findIndex(o=>o.id===t);c.splice(r,1),m===t&&(m=c[0].id,y()),k(),f()},ot=()=>{const t=T();!t||!t.trashedTasks?.length||window.confirm("Empty trash for this board?")&&(t.trashedTasks=[],k(),f())},nt=()=>{localStorage.setItem(Z,String(g))},it=()=>{const t=Number(localStorage.getItem(Z));Number.isFinite(t)&&(g=q(t,H,R))},q=(t,e,r)=>Math.min(r,Math.max(e,t)),dt=t=>{const e=-t*F;return q(e,-.04,.04)},ct=()=>{const t=document.querySelector("[data-zoom-value]");t&&(t.textContent=`${Math.round(g*100)}%`)},lt=()=>{const t=document.querySelector("[data-zoom-indicator]");t&&(t.classList.add("is-visible"),clearTimeout(O),O=window.setTimeout(()=>{t.classList.remove("is-visible")},900))},G=t=>{document.body.classList.toggle("is-dragging-task",t)},W=()=>{x?.remove(),x=null},ut=t=>{W();const e=t.cloneNode(!0);return e.classList.add("task-card-drag-preview"),e.style.width=`${t.offsetWidth}px`,e.style.height=`${t.offsetHeight}px`,e.style.position="fixed",e.style.top="-9999px",e.style.left="-9999px",e.style.transform=`scale(${g})`,e.style.transformOrigin="top left",e.querySelectorAll("input, textarea, button").forEach(r=>{r.setAttribute("tabindex","-1")}),document.body.append(e),x=e,e},I=()=>{h=null,A?.classList.remove("is-dragging-source"),A=null,W(),G(!1),document.querySelector("[data-trash-zone]")?.classList.remove("is-over")},j=({showIndicator:t=!1}={})=>{const e=document.querySelector("[data-board]");e&&(e.style.setProperty("--zoom",g.toString()),ct(),t&&lt())},$=(t,{showIndicator:e=!0}={})=>{g=q(t,H,R),nt(),j({showIndicator:e})},mt=()=>{const t=T();if(!t)return;const e=t.nextTaskNumber;t.nextTaskNumber+=1,u.push(U(e)),v(),f()},M=(t,e,r)=>{const o=u.find(i=>i.id===t);o&&(o[e]=r,v())},pt=t=>{const e=u.findIndex(i=>i.id===t);if(e===-1)return;const[r]=u.splice(e,1),o=T();o&&r&&(o.trashedTasks=o.trashedTasks??[],o.trashedTasks.push({...r,deletedAt:Date.now()})),v(),f()},ft=t=>{const e=u.find(r=>r.id===t);e&&(e.completed=!e.completed,v(),f())},z=(t,e)=>{const r=u.find(i=>i.id===t),o=e.trim();!r||!o||(r.notes.push({text:o,createdAt:Date.now()}),v(),f())},gt=(t,e)=>{const r=u.findIndex(i=>i.id===t);if(r===-1||e<0||e>=u.length||r===e)return;const[o]=u.splice(r,1);u.splice(e,0,o),v(),f()},bt=t=>`
...
`,f=()=>{const t=T(),e=u.filter(a=>a.completed),r=t?.trashedTasks??[];V.innerHTML=`
    <main class="shell">
      <aside class="left-menu" aria-hidden="false">
        ...
        <section class="menu-section">
          <div class="menu-section-head">
            ...
            <button class="menu-section-action" type="button" data-empty-trash ${r.length?"":"disabled"}>Empty</button>
          </div>
          <ul class="menu-task-list">
